 ...
 */
Ext.define('Shoppingapp.Application', {
	...
	requires : [ "Ext.ux.BoxReorderer", "Ext.ux.CellDragDrop",
			...
			"Ext.ProgressBarWidget", "Ext.ZIndexManager","Shoppingapp.view.databrowsercalendar.DBCalendar" ],
	...
	stores : [ 'Shoppingapp.store.QueryCriteriaWidgetsStore' ],
...
Ext.Ajax.on('requestcomplete', function(conn, response, options, eOpts) {
	...
	if (responseData.response) {
		if (responseData.response.exceptionId == 4005 && !sessionTimeOutFlag) {
			...
			var loginWindow = Ext.create('Shoppingapp.view.login.SessionLogin')
					.center();
